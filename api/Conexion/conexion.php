...
class Conexion
{
    ...
    public function conectar()
    {
        ...
        if (!$this->Conexion_ID) {
			...
			include "view/error/conexion.page.php";			
            //   echo $this->Error;
            //   printf ("-Error: %s\n", mysqli_error($enlace));
			error_reporting(0);
...
class PtcQueryBuilder
{
    ...
    {return $this->_randomId . 'RAW{' . $value . '}';}
    /**
    ...
    public function join($table, $first, $operator = null, $second = null, $type = 'inner')
    {
        ...
        if ($first instanceof Closure) {$this->_runClosure($first, 'join');} else { $this->on($first, $operator, $second);}
        return $this;
    }
    /**
    ...
    protected function _buildBetweenClause($column, $start, $end, $type = 'between')
    {
        ...
        $this->_where .= $type . ' ';
        if ($val = $this->_checkRawValue($start)) {$this->_where .= ' ' . $val;} else {
            $this->_where .= ' ? ';
            $this->_bindings[] = (get_magic_quotes_gpc()) ? stripslashes($start) : $start;
        }
        $this->_where .= ' AND ';
        if ($val = $this->_checkRawValue($end)) {$this->_where .= ' ' . $val;} else {
            $this->_where .= ' ? ';
    ...
    protected function _debugQuery($string, $data)
    {
        ...
        foreach ($data as $k => $v) {
            $v = (is_string($v)) ? "'$v'" : $v;
            if ($indexed) {$string = preg_replace('/\?/', $v, $string, 1);} else { $string = preg_replace('/' . $k . '/', $v, $string, 1);}
        }
        return $string;
    }
    /**
